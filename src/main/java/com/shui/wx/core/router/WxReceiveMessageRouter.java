package com.shui.wx.core.router;
...
import com.shui.wx.beans.xml.message.receive.event.*;
import com.shui.wx.beans.xml.message.receive.event.menu.WxMenuClientEventMessage;
import com.shui.wx.beans.xml.message.receive.event.menu.WxMenuPushEventMessage;
import com.shui.wx.beans.xml.message.receive.image.WxReceiveImageMessage;
import com.shui.wx.beans.xml.message.receive.link.WxReceiveLinkMessage;
import com.shui.wx.beans.xml.message.receive.location.WxReceiveLocationMessage;
import com.shui.wx.beans.xml.message.receive.text.WxReceiveTextMessage;
import com.shui.wx.beans.xml.message.receive.video.WxReceiveShortVideoMessage;
import com.shui.wx.beans.xml.message.receive.video.WxReceiveVideoMessage;
...
/**
 * 微信消息路由
 * 当被动接收微信服务器发送过来的XML消息时，经过此方法的路由后返回完整的解析后的XML JavaBean供业务层进行业务开发
 */
@Slf4j
public class WxReceiveMessageRouter {
     ...
     * 3、后续解决了密文解密支持多公众号接入的问题后，此方法将xml密文解密为明文，直接调用routerPlaintext(String message)方法即可<br/>
     *
     * @param xml 原始xml字符串
     * @return 路由结果
     */
    private static WxReceiveRouterResult routerCrypt(String xml) {
        ...
        //TODO 后续解决了密文解密支持多公众号接入的问题后，此方法将xml密文解密为明文，直接调用routerPlaintext(String message)方法即可。
        return null;
    }
