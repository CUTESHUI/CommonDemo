...
public class FanoutRabbitMQListener {
    ...
    public void fanoutBListenter(Message message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) Long tag) throws Exception {
        if (!Objects.isNull(message.getBody())) {
            log.info("message:{}", new String(message.getBody(), StandardCharsets.UTF_8));
            channel.basicAck(tag, false);
        }
    ...
    public void fanoutCListenter(Message message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) Long tag) throws Exception {
        if (!Objects.isNull(message.getBody())) {
            log.info("message:{}", new String(message.getBody(), StandardCharsets.UTF_8));
            channel.basicAck(tag, false);
        }
